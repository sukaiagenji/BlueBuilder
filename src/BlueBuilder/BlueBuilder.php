...
class BlueBuilder extends PluginBase implements Listener{
    ...
    public function onCommand(CommandSender $sender, Command $cmd, $label, array $args){
        ...
        if($cmd->getName() == "bluebuilder"){
            if(!isset($args[0])) {
                $sender->sendMessage("BlueBuilder was created by moebius2033 for use with PocketMine API version 3.0.0-ALPHA1.");
    ...
    public function saveAreas() {
        ...
        $i = 0;
        foreach($this->bluePrints as $name => $blockList) {
            $bluePrints[$i][$name] = $blockList;
            $i++;
        }
        file_put_contents($this->getDataFolder() . "blueprints.json",json_encode($bluePrints));
    }
